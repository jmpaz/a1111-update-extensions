...
# Check that the extensions directory exists, and that it contains subfolders
if [ ! -d "$EXTENSIONS_PATH" ]; then
    echo "Extensions directory $EXTENSIONS_PATH does not exist."
    exit 1
fi
if [ "$(ls -A $EXTENSIONS_PATH)" ]; then
    echo "Extensions directory $EXTENSIONS_PATH is empty."
    exit 1
fi

# Iterate over all subdirectories of the extensions folder.
declare –a UPDATED_EXTENSIONS=()
...
do
    ...
    if [ "$CURRENT_COMMIT" != "$NEW_COMMIT" ]; then
        UPDATED_EXTENSIONS +=("$dir $CURRENT_COMMIT $NEW_COMMIT")
    fi
done
